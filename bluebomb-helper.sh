...
download() {
    ...
    wget -q --secure-protocol=TLSv1_2 "https://github.com/RTG28/bluebomb/raw/download/bluebomb1.5.zip" -O bluebomb.zip
    printf "Success!\n\n* Unpacking BlueBomb... "
...
findinfos() {
    ...
    if [[ -z $consoleIn ]]; then
        printf "Qual é o tipo de console, ou erro?\n"
        ask 0 "Wii" "Wii mini" "Wii erro 003"
    else answer=$consoleIn; unset consoleIn; fi
    case ${answer^^} in
        1 | "WII" ) arg1="WII_SM" ;;
        2 | "WII MINI" ) arg1="MINI_SM_" ;;
        3 | "WII ERRO 003" ) arg1="WII_SM" ;;
        * ) printf "Invalid selection.\n"; sleep 2; findinfos ;;
    esac
    if [[ $arg1 == "MINI_SM_" ]]; then
        if [[ -z $regionIn ]]; then
            sc 2 "Collecting information - Console region"
            printf "Qual é a região do seu console Wii Mini?\n"
            ask 0 "USA" "PAL"
        ...
        if [[ -z $sysmenuIn ]]; then
            ...
            printf "Qual é a versão do menu do sistema do seu console Wii?\n\nSe você não tiver certeza de onde encontrar isso, siga as instruções:\n\nNo menu Wii, inicie 'Wii Options',e em seguida inicie 'Wii Settings', na parte superior canto direito, você deverá ver 'Ver X.XX', Um exemplo '4.3E'. para o erro 003 digite 0.3U para NTSC, ou 0.3E para PAL\n\nObs:.para o erro 003,são necessário\n1-ModChip no leitor\n2-Controle Original do Gamecube ou SaveMii\n3-Jogo com iso Modificada\n4-Arquivo Boot.elf extraido da (HackMii Installer v1.2) para raiz do Pendrive \n\n\nInsira a versão do menu do sistema ou erro e pressione ENTER: " | fold -s -w "$(tput cols)"
            read -r sysmenuIn
        fi
        if [[ ! ${#sysmenuIn} == 4 ]]; then printf "Opção invalida.\n" && unset sysmenuIn && sleep 2 && findinfos;
        fi
        sysmenuVersions=("2_0E" "2_0J" "2_0U" "2_1E" "2_2E" "2_2J" "2_2U" "3_0E" "3_0J" "3_0U" "3_1E" "3_1J" "3_1U" "3_2E" "3_2J" "3_2U" "3_3E" "3_3J" "3_3U" "3_4E" "3_4J" "3_4U" "3_5K" "4_0E" "4_0J" "4_0U" "4_1E" "4_1J" "4_1K" "4_1U" "4_2E" "4_2J" "4_2K" "4_2U" "4_3E" "4_3J" "4_3K" "4_3U" "0_3U" "0_3E")
        arg2=${sysmenuIn//./_}
        arg2=${arg2^^}
...
confirm() {
    sc 3 "Confirmation"
    printf "Tudo bem, se tudo estiver correto iremos continuar:\n\n* Tipo de Console:\t\t\033[1;96m"
    case "$arg1" in
        "WII_SM" ) printf "Wii\033[0m\n" ;;
        "MINI_SM_" ) printf "Wii mini\033[0m\n" ;;
    esac
    [[ "$arg1" == "WII_SM" ]] && printf "* Versão do menu do Sistema:\t\033[1;96m%s\033[0m\n" "${arg2//_/.}" || printf "* Região do Console:\t\t\033[1;96m%s\033[0m\n" "$arg2"
    printf "\nBlueBomb será executado com as informações fornecidas acima.\n"
    ask 1 "Você deseja continuar?"
    case $response in
        0 ) execute ;;
...
execute() {
    ...
    printf "* Parando o serviço Bluetooth... (pode ser solicitada sua senha)\n"
    case $init in
    ...
    printf "\n\nSe o BlueBomb não funcionou, diga \033[1mNÃO\033[0m para a pergunta a seguir e execute o script novamente. Seja persistente, podem ser necessárias várias tentativas para funcionar.\n\n" | fold -s -w "$(tput cols)"
    ask 1 "Deseja reativar o serviço Bluetooth?"
    case $response in
        0 ) task="Habilite o serviço Bluetooth $init"
            case $init in
                "systemd" ) $sudo0 systemctl enable --now bluetooth.service ;;
                "openrc" ) $sudo0 rc-service bluetooth start ;;
            esac ;;
    esac
    printf "\n\nExiting in 1 seconds..."; sleep 1
    credit
}
...
ask() {
    case $1 in
        0 ) unset answer; shift; printf "\nOpções:\n"; for o in "$@"; do
            ...
            printf "\nPor favor selecione uma opção e pressione ENTER: " | fold -s -w "$(tput cols)"
            read -r answer
            ;;
        1 ) unset response; shift; printf "\n\033[1m%s\033[0m [\033[1;32mSIM\033[0m/\033[1;31mNÃO\033[0m]: " "$1"
            read -r answer
            case ${answer^^} in
                "S" | "SIM" ) response=0; return ;;
                "N" | "NÃO" ) response=1; return ;;
